...
/// <summary>
/// Four-tap multitap delay: equally spaced taps up to TimeMs with shared feedback and mix.
/// </summary>
public sealed class Delay4Effect : IAudioEffect
{
    public const string TypeId = "delay4";

    string IAudioEffect.TypeId => TypeId;

    private const int TapCount = 4;
    private const double MaxDelaySeconds = 2.0;

    public bool Enabled { get; set; } = true;

    public double TimeMs { get; set; } = 400.0;
    public double Feedback { get; set; } = 0.25;
    ...
    public IAudioEffect Clone() => new Delay4Effect
    {
    ...
    public void Process(Span<float> buffer)
    {
        ...
        var maxDelay = Math.Clamp((int)(TimeMs / 1000.0 * _sampleRate), TapCount, lineSize - 1);
        Span<int> taps = stackalloc int[TapCount];
        for (var t = 0; t < TapCount; t++)
            taps[t] = Math.Clamp(maxDelay * (t + 1) / TapCount, 1, lineSize - 1);
        ...
        for (var frame = 0; frame < frames; frame++)
        {
            ...
            for (var c = 0; c < channels; c++)
            {
                var dry = buffer[i + c];
                float wet = 0;
                for (var t = 0; t < TapCount; t++) wet += lines[c].ReadInt(taps[t]);
                wet /= TapCount;
                buffer[i + c] = dry * (1 - mix) + wet * mix;
                lines[c].Write(dry + wet * fb);
            }
        }
    }
}
