...
public sealed class SfzLoader
{
    ...
    private SamplerLoadResult Build(string text, string absSfzPath, IProgress<double>? progress)
    {
        ...
        // Resolve each region's sample once using that region's active default_path
        // (keyswitch banks redefine <control> default_path per articulation).
        var keyToResolved = new Dictionary<string, string>(StringComparer.Ordinal);
        ...
        foreach (var region in document.Regions)
        {
            ...
            var key = region.SampleKey;
            if (keyToResolved.ContainsKey(key)) continue;
            var defaultPath = region.DefaultPath.Length > 0 ? region.DefaultPath : document.Control.DefaultPath;
            var resolved = ResolveFile(baseDir, defaultPath, op);
            if (resolved is null) { missing.Add($"{defaultPath}{op}"); continue; }
            keyToResolved[key] = resolved;
            var force = forceResident.Contains(key);
        ...
        // Map every sample key to its loaded sample.
        var byKey = new Dictionary<string, SamplerSample>(StringComparer.Ordinal);
        foreach (var (key, resolved) in keyToResolved)
        {
            if (loaded.TryGetValue(resolved, out var s)) byKey[key] = s;
            else missing.Add(key.Replace("\0", ""));
        }

        var library = new SamplerSampleLibrary(byKey);
        var regions = BuildRegions(document, library);

        SfzOpcodeCatalog.CollectWarnings(document.Regions.Select(r => r.Opcodes.Raw), warnings);
        if (document.Regions.Any(r => r.Opcodes.Has("effect1") || r.Opcodes.Has("effect2")
                                      || r.Opcodes.Has("effect3") || r.Opcodes.Has("effect4")))
            warnings.Add("SFZ effect sends (effect1–4) are accepted but not routed (no effect bus).");
        ...
        return new SamplerLoadResult
        {
            ...
            Warnings = warnings,
            NoteOffset = document.Control.NoteOffset,
            OctaveOffset = document.Control.OctaveOffset,
            InitialCcValues = document.Control.InitialCcValues,
    ...
    public static IReadOnlyList<SamplerRegion> BuildRegions(SfzDocument document, SamplerSampleLibrary library)
    {
        ...
        foreach (var region in document.Regions)
        {
            var rt = SfzRegionBuilder.Build(region, library.Get(region.SampleKey));
    ...
    private static HashSet<string> CollectRandomAccessSamples(SfzDocument document)
    {
        ...
        foreach (var region in document.Regions)
        {
            ...
            if (reverse || (loop is not null && loop != "no_loop")) set.Add(region.SampleKey);
        }
