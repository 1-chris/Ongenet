...
/// <summary>
/// One sounding region within an <see cref="SfzInstrument"/>. Resamples its region's sample (4-point
...
public sealed class SfzVoice
{
    ...
    public SfzStream Stream { get; } = new();
    ...
    public SfzTrigger Trigger => _rt?.Trigger ?? SfzTrigger.Attack;

    public void Start(SfzRegionRuntime rt, int triggerNote, int velocity, double extraSemis,
        SfzModState mod, AudioFormat format)
    {
        _rt = rt;
        _sample = rt.Sample;
    ...
    private void SetupModulation(SfzRegionRuntime rt, int triggerNote, int velocity)
    {
    ...
    public void Release()
    {
        if (_loopMode == SfzLoopMode.OneShot) return; // one_shot plays to the end regardless of note-off
        _released = true;
        _env.Release();
    ...
    public void Render(Span<float> buffer)
    {
        ...
        _looping = _loopMode is SfzLoopMode.LoopContinuous || (_loopMode == SfzLoopMode.LoopSustain && !_released);
