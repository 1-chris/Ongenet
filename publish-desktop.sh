...
ALL_RIDS="linux-x64 win-x64 osx-arm64 osx-x64"
SYMBOLS=0
DO_ZIP=1
DO_NATIVE=1
RIDS=""
...
for arg in "$@"; do
    case "$arg" in
        ...
        linux-x64|win-x64|osx-arm64|osx-x64) RIDS="$RIDS $arg" ;;
...
native_file() {
    case "$1" in
        linux-x64) echo "libportaudio.so.2" ;;
...
for rid in $RIDS; do
    ...
    if [ "$rid" = "linux-x64" ] && [ -f "$out/Ongenet.Desktop" ]; then
