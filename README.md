...
| ------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Ongenet.Core`                  | none                    | The heart of the app, fully platform-agnostic. Audio models (project / tracks / clips / MIDI notes), the lock-free audio **engine** (sequencer, per-track mixing, metering, automation), the **instrument** framework (~24 built-ins including synths, SFZ/SF2 sampler, drums, containers) and **effects** chain (~90 built-ins including mastering, dynamics, spectral, visualisers…), the shared DSP toolkit (`Audio/Dsp`), the parameter framework, WAV decode/encode, a cross-platform **MIDI** model (running-status parser, learn/transport mappings), and the app services (project, transport, selection, recording, edit-mode, MIDI input/mapping) plus DI registration, an in-process event aggregator, and logging. Depends only on the BCL. |
...
| `Ongenet.Au`                    | AU plugins (macOS)      | macOS **Audio Unit** hosting via the Component Manager — music devices as instruments, effects in the chain, native Cocoa editor embedding. Bridged into Core's registries at runtime; none required.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `Ongenet.PluginHost`            | VST3 (child process)    | Headless child executable for optional plugin crash isolation. Communicates with the desktop host over named pipes; loads VST3 effects out-of-process when **Settings → General → Plugins → Run VST3 plugins in an isolated process** is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `Ongenet.Desktop`               | Avalonia (+ all native) | The **desktop head**: a thin exe that wires the native stack — Avalonia desktop backends, `Ongenet.Audio`, the CLAP/LV2/VST/AU plugin hosts, `Ongenet.Scripting`, `Ongenet.PluginHost`, and the `Ongenet.Engine3D` GPU engine — into the shared `Ongenet.App` UI via `DesktopPlatform`. MVVM, DI bootstrap, the classic `MainWindow`. Publishes as `Ongenet`.                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
|---|---|
| Modular **Grid** / FL **Patcher** | **Field** (instrument + effect) |
...
| Field modular engine                        | **Strong** — modular grid-class patching as instrument and effect                                                   |
| Comping / take lanes                        | **Good** — multi-lane loop recording, toggle comp regions, crossfade flatten, warp-aware bake                        |
| Export delivery                             | **Strong** — WAV/FLAC/MP3/OGG, stems, surround (5.1/7.1), ADM BWF + XML sidecar, AAF/OMF XML handoff, timeline XML, composited MP4; master LUFS/LRA/true-peak analysis, embedded loudness tags, platform normalisation, TPDF/noise-shaped dither, album matching, windowed-sinc SRC; see [mixer & export guide](site/articles/guides/mixer-and-export.md#mastering-on-the-master-track) |
...
Master delivery uses BS.1770-style loudness plus a 4× polyphase-FIR true-peak meter. Eight core mastering chain recipes plus **Audiophile Master** and **Reference Master** (ten named recipes in `MasteringChains`) share the FX Chains library. The mastering
clipper and Peak Limiter 2×/4× modes perform real FIR upsample → high-rate process → downsample DSP;
they are not sample-peak-only, Hermite, or zero-order-hold ISP approximations.
...
| Content library                             | **Good** — factory Content/Core (~1.4 GB): Stargate/Oramics + VCSL kit/Strumstick/Kawai piano + VSCO2CE orchestra + tonejs CC-BY bass/organ/xylo, SFZ kits + Inst/FX factory presets + demo songs |
...
**Startup project:** a fresh **desktop** launch opens **Ascension** (uplifting trance); **WASM** and **Android** open the lightweight **Web Demo** sketch instead. **First Light** is a deep progressive house library demo — open it from Projects, not the default startup song.

Each built-in project ships with a genre-appropriate master chain from [`MasteringChains`](Ongenet.Core/Audio/Effects/MasteringChains.cs) — typically **Full Master**, **Streaming Master**, **Club Loud**, or **Techno Master** (see [site/articles/guides/mastering.md](site/articles/guides/mastering.md) for the full matrix).
...
**Twenty-four** built-in instruments ship in `Ongenet.Core` (`Audio/Instruments`), registered in
`InstrumentRegistry`, plus the **Field** modular instrument registered at startup. Any CLAP, LV2, VST2,
VST3, or AU (macOS) instrument you have installed is discovered at runtime and appears alongside them.


| Category | Instruments |
| -------- | ----------- |
| **Synths** | Oscillator, 3x Osc, Wavetable, FM Synth, Bass Synth, Organ, Phase-4, Polymer, Polysynth, Padda |
| **Samplers** | Basic Sampler, Sampler (SFZ/SF2), Granular |
| **Drums** | Kicka, Perca, Drum Model |
| **Hardware** | HW Instrument, HW CV Instrument |
| **Containers** | Drum Machine, Instrument Layer, Instrument Selector, Chain, XY Instrument, Replacer |
| **Modular** | **Field** — node-graph instrument (also available as an effect). See [docs/creating-field-nodes.md](docs/creating-field-nodes.md). |


Highlights: **Padda** is a lush pad with an internal drive → chorus → delay → reverb chain; **Kicka** /
**Perca** cover kick and percussion one-shots; **Sampler** maps multi-sample SFZ/SF2 patches with velocity
layers and round-robin; **Field** patches oscillators, envelopes, filters, and module nodes on a zoomable
canvas (built-in presets reconstruct other instruments as editable graphs).
...
**Ninety** built-in effects ship in `Ongenet.Core` (`Audio/Effects`), registered in `EffectRegistry` and
grouped by category, plus the **Field** modular effect registered at startup. CLAP, LV2, VST2, VST3, and
AU effects are likewise discovered at runtime and slot into the same chain.


| Category           | Effects (representative) |
| ------------------ | ------------------------ |
| **EQ & Filter**    | EQ, EQ-2/5/+, EQ-DJ, Tilt, Filter/+, Ladder, Comb, Focus, Sculpt, Resonator Bank, Sweep |
| **Dynamics**       | Compressor/+, Dynamics, Limiter (bus), Gate, Sidechain, De-Esser, Transient Control |
| **Mastering**      | Mid/Side EQ, Match EQ, Linear-Phase EQ, Multiband (OTT), Peak Limiter, Clipper, Stereo Width, Spectrum, Loudness Meter, Tool |
| **Modulation**     | Chorus/+, Phaser/+, Flanger/+, Tremolo, **Stuttero**, Rotary, Blur, Treemonster |
| **Delay & Reverb** | Delay/1/2/4/+, Reverb, Convolution |
| **Distortion**     | Distortion, Saturator, Amp, Over, Exciter, Bitcrusher |
| **Pitch**          | Vocoder, Auto-Tune, Pitch Shifter, Freq Shifter/+, Ring-Mod, Time Shift |
| **Utility**        | Dual Pan, DC Offset, Live Difference, Utility, Test Tone, Tuner |
| **Visualizer**     | **3D Scope**, Oscilloscope |
| **Spectral / hardware / containers** | Freq/Harmonic/Loud/Transient Split; HW FX/CV/Clock; FX Layer/Selector, Multiband FX, Mid-Side/Stereo Split, XY FX, receivers |
...
| [site/articles/guides/mastering.md](site/articles/guides/mastering.md) | Master chains, loudness, Compare, delivery targets, export |
